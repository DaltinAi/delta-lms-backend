...
describe('InvitationsService', () => {
  ...
  describe('createInvitation()', () => {
    ...
    it('should create an invitation and return invite link', async () => {
      mockDbQuery
        ...
        .mockResolvedValueOnce({
          rows: [{ id: 'inv-1', email: dto.email, role: 'telecaller', token: 'tok', expires_at: new Date() }],
        }); // insert

      const result = await service.createInvitation('cid', 'inviter-id', dto as any);
    ...
    it('should throw 409 when user with email already exists in company', async () => {
      ...
      await expect(service.createInvitation('cid', 'uid', dto as any)).rejects.toMatchObject({ status: 409 });
    });
  });
  ...
  describe('acceptInvitation()', () => {
    const dto = { token: 'valid-token', firstName: 'John', lastName: 'Doe', password: 'Pass@123' };

    it('should accept invitation and create user', async () => {
      const futureDate = new Date(Date.now() + 86400000);
      const invite = { id: 'inv-1', company_id: 'cid', email: 'new@acme.com', role: 'telecaller', expires_at: futureDate };

      const mockClient = {
        query: jest.fn()
          .mockResolvedValueOnce({ rows: [invite] })  // token lookup
          .mockResolvedValueOnce({ rows: [{ id: 'uid-new', email: 'new@acme.com', role: 'telecaller' }] }) // insert user
          .mockResolvedValueOnce({ rows: [] }), // mark used
      };

      mockDbTransaction.mockImplementationOnce((cb: any) => cb(mockClient));

      const result = await service.acceptInvitation(dto as any);
    ...
    it('should throw 404 for invalid or used token', async () => {
      ...
      await expect(service.acceptInvitation(dto as any)).rejects.toMatchObject({ status: 404 });
    });

    it('should throw 400 for expired invitation', async () => {
      const pastDate = new Date(Date.now() - 86400000);
      const invite = { id: 'inv-1', company_id: 'cid', email: 'new@acme.com', role: 'telecaller', expires_at: pastDate };
      ...
      await expect(service.acceptInvitation(dto as any)).rejects.toMatchObject({ status: 400 });
    });
  });
  ...
  describe('getPendingInvitations()', () => {
    it('should return list of pending invitations', async () => {
      const rows = [{ id: 'inv-1', email: 'a@b.com' }, { id: 'inv-2', email: 'c@d.com' }];
