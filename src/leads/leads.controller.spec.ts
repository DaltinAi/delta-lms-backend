...
describe('LeadsController', () => {
  ...
  describe('createLead()', () => {
    it('should create and return a new lead', async () => {
      ...
      const result = await controller.createLead({ firstName: 'Jane', phone: '999' } as any, mockUser);
      expect(result).toEqual({ status: 201, message: 'Lead created successfully', data: lead });
    });

    it('should propagate errors from service', async () => {
      mockLeadsService.createLead.mockRejectedValueOnce({ status: 500, message: 'DB error' });
      await expect(controller.createLead({} as any, mockUser)).rejects.toMatchObject({ status: 500 });
    });
  });
  ...
  describe('getLeads()', () => {
    it('should return paginated leads', async () => {
      mockLeadsService.getLeads.mockResolvedValueOnce({ data: [{ id: 'l1' }], total: 1 });
  ...
  describe('checkPhone()', () => {
    it('should return exists: true when phone found', async () => {
      mockLeadsService.checkPhoneExists.mockResolvedValueOnce({
        exists: true, leadId: 'l1', name: 'John Doe',
      });

      const result = await controller.checkPhone('9876543210', mockUser);
      expect(result).toEqual({ status: 200, exists: true, leadId: 'l1', name: 'John Doe' });
    });

    it('should return exists: false when phone not found', async () => {
      mockLeadsService.checkPhoneExists.mockResolvedValueOnce({ exists: false });
    ...
    it('should throw 400 when phone query param is missing', async () => {
      await expect(controller.checkPhone('', mockUser)).rejects.toMatchObject({ status: 400 });
    });
  });
  ...
  describe('getLeadById()', () => {
    ...
    it('should throw 404 when lead not found', async () => {
      ...
      await expect(controller.getLeadById('missing')).rejects.toMatchObject({ status: 404 });
    });
  });
  ...
  describe('updateLead()', () => {
    it('should update and return the lead', async () => {
      ...
      const result = await controller.updateLead('l1', { firstName: 'Updated' } as any, mockUser);
      expect(result).toEqual({ status: 200, message: 'Lead updated successfully', data: updated });
    });

    it('should throw 404 when lead not found for update', async () => {
      mockLeadsService.updateLead.mockResolvedValueOnce(null);
      await expect(controller.updateLead('missing', {} as any, mockUser)).rejects.toMatchObject({ status: 404 });
    });
  });
  ...
  describe('updateLeadStage()', () => {
    it('should update stage and return result', async () => {
      mockLeadsService.updateLeadStage.mockResolvedValueOnce({ success: true, message: 'Stage updated successfully' });

      const result = await controller.updateLeadStage('l1', { toStageId: 's1', remark: 'ok' } as any, mockUser);
  ...
  describe('reassignLeads()', () => {
    it('should reassign and return result', async () => {
      mockLeadsService.reassignLeads.mockResolvedValueOnce({ success: true, message: 'Reassigned 2 leads' });

      const result = await controller.reassignLeads({ leadIds: ['l1', 'l2'], toAssigneeId: 'tc-1' } as any, mockUser);
