...
describe('VisitHistoryService', () => {
  ...
  describe('createVisit()', () => {
    const dto = { leadId: 'lead-1', visitDate: '2026-07-17', notes: 'Walk-in visit' };
    ...
    it('should create a visit and return it', async () => {
      ...
      const result = await service.createVisit('cid', 'uid', dto as any);
      expect(result).toEqual(visit);
    });

    it('should throw 404 when lead not found for company', async () => {
      mockDbQuery.mockResolvedValueOnce({ rows: [] });
      await expect(service.createVisit('cid', 'uid', dto as any)).rejects.toMatchObject({ status: 404 });
    });
    ...
    it('should store null notes when not provided', async () => {
      ...
      const result = await service.createVisit('cid', 'uid', { leadId: 'lead-1', visitDate: '2026-07-17' } as any);
